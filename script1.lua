...
for _,v in next, folder:GetChildren() do
	if v:IsA('Model') then
		if v:FindFirstChildWhichIsA('Model') and v.Name ~= 'Armor' then --well the script works fine with it, the reason im properly welding everything is because of the yk
...
coroutine.resume(coroutine.create(function() wait(.5)
	for i, v in pairs(Character:GetChildren()) do
		...
		if v:IsA("Accessory") or v:IsA('Hat') then
			...
			if v:FindFirstChild('Handle') then -- issues with yk hats
